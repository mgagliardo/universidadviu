...
//Added my ourself
#include <cmath>			//to calculate satd
#include "support.cu"
#include "kernel.cu"
...
void printout(FILE* outputframe, ece408_intra_pred_result *myref);
...
/******************************************************************************************
 ******************************************************************************************
 ******************************* ECE 408 Competition Functions ****************************
 ******************************************************************************************
 *****************************************************************************************/
void init_ref_pixels_unfilt(uint8_t *channel_pix_values, uint8_t *ref_above, uint8_t *ref_left, int img_width, int img_height,
						unsigned int blk_size, unsigned int fst_rpix_idx, unsigned int fst_cpix_idx) {

	// on the top left corner
	if(fst_rpix_idx == 0 && fst_cpix_idx == 0) {
		memset(ref_above, 128, (2*blk_size+1)*sizeof(uint8_t));
		memset(ref_left, 128, (2*blk_size+1)*sizeof(uint8_t));
	}
	// on the top boundary
	else if(fst_rpix_idx == 0 && fst_cpix_idx != 0) {
		unsigned int p = fst_cpix_idx-1;
		unsigned int t_r = fst_rpix_idx;

		memset(ref_above, channel_pix_values[p], (2*blk_size+1)*sizeof(uint8_t));
		ref_left[0] = channel_pix_values[p]; // from index -1
		for(unsigned int i = 1; i < (2*blk_size+1); i++) {
			if(t_r < img_height) {
				ref_left[i] = channel_pix_values[p];
				t_r++;
			}
			else
				ref_left[i] = ref_left[i-1];
			p += img_width;
		}
	}
	// on the left boundary
	else if(fst_rpix_idx != 0 && fst_cpix_idx == 0) {
		unsigned int p = (fst_rpix_idx-1) * img_width;
		unsigned int t_c = fst_cpix_idx;
		
		memset(ref_left, channel_pix_values[p], (2*blk_size+1)*sizeof(uint8_t));
		ref_above[0] = channel_pix_values[p]; // from index -1
		for(unsigned int i = 1; i < (2*blk_size+1); i++) {
			if(t_c < img_width) {
				ref_above[i] = channel_pix_values[p];
				t_c++;
			}
			else
				ref_above[i] = ref_above[i-1];
			p++;
		}
	}
	// NOT on any boundary
	else {
		unsigned int p_above = (fst_rpix_idx-1) * img_width + fst_cpix_idx;
		unsigned int p_left = fst_rpix_idx * img_width + fst_cpix_idx-1;
		unsigned int t_r = fst_rpix_idx;
		unsigned int t_c = fst_cpix_idx;

		ref_above[0] = channel_pix_values[p_above-1]; // from index -1
		ref_left[0] = channel_pix_values[p_above-1]; // from index -1
		for(unsigned int i = 1; i < (2*blk_size+1); i++) {
			if(t_c < img_width) {
				ref_above[i] = channel_pix_values[p_above];
				t_c++;
			}
			else
				ref_above[i] = ref_above[i-1];
			...
			if(t_r < img_height) {
				ref_left[i] = channel_pix_values[p_left];
				t_r++;
			}
			else
				ref_left[i] = ref_left[i-1];
...
ece408_intra_pred_result *ece408_competition(ece408_frame *imgs, int num_frames) {
	...
	uint8_t ref_above[3*32];
	uint8_t ref_left[3*32];
	

	/****************** create ece408_intra_pred_result structure *************/
	for(int i = 0; i < num_frames; i++) {
		for(int luma_size_shift = 2; luma_size_shift <= 5; luma_size_shift++) {
	        unsigned int luma_size = 1 << luma_size_shift; // luma_size x luma_size luma PBs
	        cur_result->create(imgs->width, imgs->height, luma_size);
			cur_result ++; //Added by Jie
		}
	}

	FILE * com = fopen("com", "w");
	if(com == NULL)
	{
		printf("Error opening file stu\n");
		exit(1);
	}
	...
	int pixel_num = imgs->height * imgs->width;
	int max_ref_size = (pixel_num >> 2) * 3;

	//device neighbor pixel 
	uint8_t *refd_a;
	uint8_t *refd_l;
	uint8_t *refd_a_filt;
	uint8_t *refd_l_filt;
	cudaMalloc((void**)&refd_a,max_ref_size*sizeof(uint8_t));
	cudaMalloc((void**)&refd_l,max_ref_size*sizeof(uint8_t));
	cudaMalloc((void**)&refd_a_filt,max_ref_size*sizeof(uint8_t));
	cudaMalloc((void**)&refd_l_filt,max_ref_size*sizeof(uint8_t));

	//devie prediction block and original block
	uint8_t *predd_res; 
	uint8_t *origd_res;
	uint8_t *channel_pixs;
	cudaMalloc((void**)&predd_res, pixel_num * sizeof(uint8_t));
	cudaMalloc((void**)&origd_res, pixel_num * sizeof(uint8_t));
	cudaMalloc((void**)&channel_pixs, pixel_num * sizeof(uint8_t));

	//device final result 
	uint8_t *y_modes_d;
	int32_t *y_satd_d;
	int32_t *y_satd_us_d;
	int max_num_threads = pixel_num >> 4;
	cudaMalloc((void**) &y_modes_d, 35*max_num_threads*sizeof(uint8_t));
	cudaMalloc((void**)  &y_satd_d, 35*max_num_threads*sizeof(int32_t));
	cudaMalloc((void**)  &y_satd_us_d, 35*max_num_threads*sizeof(int32_t));

	//host neighbor block
	uint8_t *refh_a = (uint8_t *) malloc(max_ref_size*sizeof(uint8_t));
	uint8_t *refh_l = (uint8_t *) malloc(max_ref_size*sizeof(uint8_t));

	/************************* predict chroma Y channel *********************/
	cudaMemcpy(channel_pixs,imgs->y, pixel_num * sizeof(uint8_t),cudaMemcpyHostToDevice);
	for(int i = 0; i < num_frames; i++) {
		for(int luma_size_shift = 2; luma_size_shift <= 5; luma_size_shift++) {
			...
			//parallel
			unsigned int num_of_blk_rows = imgs->height/luma_size;
			unsigned int num_of_blk_cols = imgs->width/luma_size;
			unsigned int num_threads = num_of_blk_rows * num_of_blk_cols;
			int ref_size = num_threads * 3 *luma_size * sizeof(uint8_t);

			//prepare neighbor array and original pixel 
			for(unsigned int blk_row = 0; blk_row < num_of_blk_rows; blk_row++) {
				for(unsigned int blk_col = 0; blk_col < num_of_blk_cols; blk_col++) {
					unsigned int fst_row_pix_idx = blk_row * luma_size;
					unsigned int fst_col_pix_idx = blk_col * luma_size;
					int block_id = blk_row * num_of_blk_cols + blk_col;
					init_ref_pixels_unfilt(imgs->y, ref_above+luma_size-1, ref_left+luma_size-1, imgs->width, imgs->height, luma_size, fst_row_pix_idx, fst_col_pix_idx);

					for(int idx = luma_size-1; idx < luma_size*3; idx++) {
						refh_a[num_threads*idx + block_id] = ref_above[idx];
						refh_l[num_threads*idx + block_id] = ref_left[idx];
					}
				}
			}
			//print ref
			...
			cudaMemcpy(refd_a,refh_a,ref_size,cudaMemcpyHostToDevice);
			cudaMemcpy(refd_l,refh_l,ref_size,cudaMemcpyHostToDevice);

			cudaMemcpyToSymbol(num_threads_c, &num_threads, sizeof(int));
			
			//launch kernel
			dim3 dim_grid(((num_threads-1)/BLOCK_SIZE + 1), 1, 1);
			dim3 dim_block(BLOCK_SIZE, 1, 1);

			HEVC_kernel<<< dim_grid, dim_block >>>(refd_a+(luma_size-1)*num_threads, refd_l+(luma_size-1)*num_threads,
				refd_a_filt+(luma_size-1)*num_threads, refd_l_filt+(luma_size-1)*num_threads, channel_pixs,
				predd_res, origd_res, luma_size, (luma_size<=16), y_modes_d, y_satd_d, 1, y_satd_us_d, imgs->width, num_of_blk_cols);	

			//copy final result
			cudaMemcpy(cur_result-> y_modes,		y_modes_d,	35*num_threads*sizeof(uint8_t),cudaMemcpyDeviceToHost);
			cudaMemcpy(cur_result-> y_satd_results,	y_satd_d,	35*num_threads*sizeof(int32_t),cudaMemcpyDeviceToHost);

	        cur_result++;
	    } // for(luma_size)
	} // for(num_frames)

	/************************* predict chroma U channel *********************/
	cudaMemcpy(channel_pixs,imgs->cb, (pixel_num>>2) * sizeof(uint8_t),cudaMemcpyHostToDevice);
	cur_result = ret;
	for(int i = 0; i < num_frames; i++) {
		cur_result++;	//skip luma_size == 4
		for(int chroma_size_shift = 2; chroma_size_shift <= 4; chroma_size_shift++) {
			unsigned int chroma_size = 1 << chroma_size_shift; // luma_size x luma_size luma PBs

			//parallel
			unsigned int num_of_blk_rows = (imgs->height/2)/chroma_size;
			unsigned int num_of_blk_cols = (imgs->width/2)/chroma_size;
			unsigned int num_threads = num_of_blk_rows * num_of_blk_cols;
			int ref_size = num_threads * 3 *chroma_size * sizeof(uint8_t);

			//prepare neighbor array and original pixel 
			for(unsigned int blk_row = 0; blk_row < num_of_blk_rows; blk_row++) {
				for(unsigned int blk_col = 0; blk_col < num_of_blk_cols; blk_col++) {
					unsigned int fst_row_pix_idx = blk_row * chroma_size;
					unsigned int fst_col_pix_idx = blk_col * chroma_size;
					int block_id = blk_row * num_of_blk_cols + blk_col;
					init_ref_pixels_unfilt(imgs->cb, ref_above + chroma_size-1, ref_left+chroma_size-1, imgs->width/2, imgs->height/2, chroma_size, fst_row_pix_idx, fst_col_pix_idx);

					for(int idx = chroma_size-1; idx < chroma_size*3; idx++) {
						refh_a[num_threads*idx + block_id] = ref_above[idx];
						refh_l[num_threads*idx + block_id] = ref_left[idx];
					}
				}
			...
			dim3 dim_block(BLOCK_SIZE, 1, 1);
			HEVC_kernel<<< dim_grid, dim_block >>>(refd_a+(chroma_size-1)*num_threads, refd_l+(chroma_size-1)*num_threads,
								refd_a_filt+(chroma_size-1)*num_threads, refd_l_filt+(chroma_size-1)*num_threads, channel_pixs,
								predd_res, origd_res, chroma_size, 0 ,y_modes_d, y_satd_d, 0, y_satd_us_d, imgs->width/2, num_of_blk_cols);	

			//copy final result
			cudaMemcpy(cur_result-> cb_modes,		y_modes_d,	35*num_threads*sizeof(uint8_t),cudaMemcpyDeviceToHost);
			cudaMemcpy(cur_result-> cb_satd_results,	y_satd_d,	35*num_threads*sizeof(int32_t),cudaMemcpyDeviceToHost);
	...
	/************************* predict chroma U channel *********************/
	cudaMemcpy(channel_pixs,imgs->cr, (pixel_num>>2) * sizeof(uint8_t),cudaMemcpyHostToDevice);
	cur_result = ret;
	for(int i = 0; i < num_frames; i++) {
		cur_result++;	//skip luma_size == 4
		for(int chroma_size_shift = 2; chroma_size_shift <= 4; chroma_size_shift++) {
			...
			for(unsigned int blk_row = 0; blk_row < num_of_blk_rows; blk_row++) {
				for(unsigned int blk_col = 0; blk_col < num_of_blk_cols; blk_col++) {
					...
					unsigned int fst_col_pix_idx = blk_col * chroma_size;
					int block_id = blk_row * num_of_blk_cols + blk_col;
					init_ref_pixels_unfilt(imgs->cr, ref_above + chroma_size-1, ref_left+chroma_size-1, imgs->width/2, imgs->height/2, chroma_size, fst_row_pix_idx, fst_col_pix_idx);
			...
			cudaMemcpy(refd_a,refh_a,ref_size,cudaMemcpyHostToDevice);
			cudaMemcpy(refd_l,refh_l,ref_size,cudaMemcpyHostToDevice);
			//host to device original pixel copy
			cudaMemcpyToSymbol(num_threads_c, &num_threads, sizeof(int));
			...
			//copy final result
			cudaMemcpy(cur_result-> cr_modes,		y_modes_d,	35*num_threads*sizeof(uint8_t),cudaMemcpyDeviceToHost);
			cudaMemcpy(cur_result-> cr_satd_results,	y_satd_d,	35*num_threads*sizeof(int32_t),cudaMemcpyDeviceToHost);
	...
	cudaFree(refd_a);
	cudaFree(refd_l);
	cudaFree(refd_a_filt);
	cudaFree(refd_l_filt);

	//device prediction block and original block
	cudaFree(predd_res);
	cudaFree(origd_res);
	cudaFree(channel_pixs);

	//device final result
	cudaFree(y_modes_d);
	cudaFree(y_satd_d);
	cudaFree(y_satd_us_d);
