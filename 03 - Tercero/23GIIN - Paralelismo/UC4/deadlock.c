...
#include <stdio.h>
#include "mpi.h"
#define MSGLEN 1024000         /* length of message in elements */
#define TAG_A 100
#define TAG_B 200
     ...
     char **argv;
{
  ...
  /* ---------------------------------------------------------------
   * send and receive messages <Using non-blocking comm>
   * --------------------------------------------------------------- */
  MPI_Request request;
  printf ( " Task %d has initiated non-blocking send\n", rank );
  MPI_Isend ( message1, MSGLEN, MPI_FLOAT, dest, send_tag, MPI_COMM_WORLD, &request );
