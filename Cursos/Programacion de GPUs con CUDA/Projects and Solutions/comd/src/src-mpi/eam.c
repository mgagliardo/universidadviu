...
extern void lunch_eamForce_kernel(float *rhobar, float *dfEmbed, float phi_n, float phi_x0, float phi_invDx, float *phi_values, 
...
extern void lunch_eamForce_kernel_2(float *dfEmbed, float rho_n, float rho_x0, float rho_invDx, float *rho_values,
...
int eamForce(SimFlat* s)
{
   ...
   //printf("nTotalBoxes = %d \n", s->boxes->nTotalBoxes);


   float *flat_r = comdMalloc((maxTotalAtoms*3)*sizeof(float));
   float *flat_f = comdMalloc((maxTotalAtoms*3)*sizeof(float));

   int z = 0;
   for(int x = 0; x < maxTotalAtoms; x++)
   {
	for (int y = 0; y < 3; y++)
	{
		flat_r[z] = s->atoms->r[x][y];
		flat_f[z] = s->atoms->f[x][y];
		z++;
        }
   }
...
//printf("f_n = %d, f_x0 = %f, f_invDx = %f \n", pot->f->n, pot->f->x0, pot->f->invDx);
//printf("phi_n = %d, phi_x0 = %f, phi_invDx = %f \n", pot->phi->n, pot->phi->x0, pot->phi->invDx);
//printf("rho_n = %d, rho_x0 = %f, rho_invDx = %f \n", pot->rho->n, pot->rho->x0, pot->rho->invDx);


float phi_n = (float)pot->phi->n;
float rho_n = (float)pot->rho->n;
float f_n = (float)pot->f->n;

lunch_eamForce_kernel((float*)pot->rhobar, (float*)pot->dfEmbed, phi_n, (float)pot->phi->x0, (float)pot->phi->invDx,  (float*)pot->phi->values ,
				                                 rho_n, (float)pot->rho->x0, (float)pot->rho->invDx,  (float*)pot->rho->values , 
				                                 f_n, (float)pot->f->x0, (float)pot->f->invDx,  (float*)pot->f->values,
                                                                 s->boxes->nLocalBoxes, s->boxes->nTotalBoxes, s->boxes->nHaloBoxes, s->boxes->nAtoms, 
				                                 s->boxes->gridSize, flat_r, (float*)s->atoms->U, flat_f, (float)rCut2, (float*)&(etot));
   ...
   for(int x = 0; x < maxTotalAtoms; x++)
   {
        for (int y = 0; y < 3; y++)
        {
                s->atoms->r[x][y] = flat_r[z];
                s->atoms->f[x][y] = flat_f[z];
                z++;
        }
   }
...
/*
   ...
   // loop oner local boxes
   //printf("nLocalBoxes = %d \n", s->boxes->nLocalBoxes);
   for (int iBox=0; iBox<s->boxes->nLocalBoxes; iBox++)
   {
      int nIBox = s->boxes->nAtoms[iBox];
      //printf("nIBOX = %d, iBox = %d \n", nIBox, iBox);
	 ...
	 //printf("nIBox = %d \n", nIBox);
	     ...
	     //printf("iOff = %d \n", iOff);
		  ...
		  //printf("iOff = %d, jOff = %d\n", iOff, jOff);
               ...
               // accumulate rhobar for each atomi
	       //printf("rhoTmp = %f\n", rhoTmp);
               pot->rhobar[iOff] += rhoTmp;
	       //printf("rhobar_d[%d] = %f\n", iOff, pot->rhobar[iOff]);
...
*/
  ...
  // LUNCH 2ND KERNEL HERE!!!!
   lunch_eamForce_kernel_2((float*)pot->dfEmbed, rho_n, (float)pot->rho->x0, (float)pot->rho->invDx,  (float*)pot->rho->values,
                           s->boxes->nLocalBoxes, s->boxes->nTotalBoxes, s->boxes->nHaloBoxes, s->boxes->nAtoms,
                           s->boxes->gridSize, flat_r, flat_f, (float)rCut2);
