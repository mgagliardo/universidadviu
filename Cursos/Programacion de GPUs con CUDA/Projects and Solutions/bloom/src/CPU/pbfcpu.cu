...
void insertWords(char *bloom, BloomOptions_t *bloomOptions_t, size_t nkeys, char *keys, unsigned int *keyPos, float prob)
{
	...
	for (i = 0; i < nkeys; ++i) {
		...
		for (j = 0; j < bloomOptions_t->numHashes; ++j) {
			unsigned long firstValue = djb2HashOffset(keys, (int)keyPos[i]) % bloomOptions_t->size;
			unsigned long secondValue = sdbmHashOffset(keys, (int)keyPos[i]) % bloomOptions_t->size;
			int value = (firstValue + (j * j * secondValue) % bloomOptions_t->size) % bloomOptions_t->size;
...
void queryWords(char *bloom, BloomOptions_t *bloomOptions_t, size_t nkeys, char *keys, unsigned int *keyPos, int *results) {	
	...
	for(i = 0; i < nkeys; ++i) {
		int j, count = 0;
		for(j = 0; j < bloomOptions_t->numHashes; ++j) {
			unsigned long firstValue = djb2HashOffset(keys, keyPos[i]) % bloomOptions_t->size;
			unsigned long secondValue = sdbmHashOffset(keys, keyPos[i]) % bloomOptions_t->size;
			int value = (firstValue + (j * j * secondValue) % bloomOptions_t->size) % bloomOptions_t->size;
			count += bloom[value];
		}				
		results[i] = count;		
