...
extern void lunch_ljForce_kernel(real_t *p, int nLB, int *nA, int *gridS, int *g, real3 *rr, real_t *Uu, real3 *ff, int sz);
...
int ljForce(SimFlat* s)
{
   ...
   //lunch_ljForce_kernel(s);

   real_t potn[7];

   //puts sigma, epsilon, the cutoff distance, the curoff distance square, 
   //and zero ePot? into array of real_t -Simon
   ...
   s->ePotential = 0.0;
   //nTotalBoxes = total number of boxes on processor -Simon
   int fSize = s->boxes->nTotalBoxes*MAXATOMS; //Total number of atoms in this proccessor -Simon
   //for every atom in this system set forces to zero and Potential energy per atom to zero -Simon
   ...
   lunch_ljForce_kernel(potn, s->boxes->nLocalBoxes, s->boxes->nAtoms, s->boxes->gridSize, s->atoms->gid, s->atoms->r, s->atoms->U, s->atoms->f, fSize);

   int nbrBoxes[27];
   // loop over local boxes
   //printf("number of local Boxes: %d \n", s->boxes->nLocalBoxes);
